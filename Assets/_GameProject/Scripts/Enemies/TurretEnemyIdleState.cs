...
public class TurretEnemyIdleState : TurretEnemyState
{
    ...
    public TurretEnemyIdleState(TurretEnemyController enemy) : base(enemy) { }
    ...
    {
        //Debug.Log("Turret enemy exiting idling state");
    }
    ...
    public override void OnStateUpdate()
    {
        ...
        if (Physics.SphereCast(_enemy._enemyEye.position, _enemy._checkRadius, _enemy.transform.forward, out RaycastHit hit, _enemy._playerCheckDistance))
        {
            if (hit.transform.CompareTag("Player"))
            {
                ...
                _enemy.ChangeState(new TurretEnemyAttackState(_enemy));
            }
        }
    }
}
